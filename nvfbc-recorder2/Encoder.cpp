...
HRESULT Encoder::LaunchEncode(unsigned int i, CUdeviceptr devptr)
{
    ...
    if (m_stInputSurface[i].hInputSurface == NULL)
    {
        ...
        {
            if (m_cuContext) {
                cuCtxPushCurrent((CUcontext)m_cuContext);
            }
			...
			std::cout << m_cuContext << " " << param.resourceToRegister << std::endl;

            if (status != NV_ENC_SUCCESS)
            {
                fprintf(stderr, __FUNCTION__": Failed to register resource with encoder.\n");
                fprintf(stderr, "NVENC Register Resource failed with status: %d\n", status);
                return E_FAIL;
            }
...
HRESULT Encoder::LoadEncAPI()
{
    ...
    if (Is64Bit())
    {
        m_hinstLib = LoadLibraryA(__NVEncodeLibName64);
    }
    else
    {
        m_hinstLib = LoadLibraryA(__NVEncodeLibName32);
    }
